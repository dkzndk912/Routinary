...
interface RoutineDao {
    @Insert // 2. 삽입 쿼리
    suspend fun insertRoutine(routine: Routine) // suspend는 코루틴 내에서 비동기 작업을 수행함을 의미

    @Query("SELECT * FROM routine ORDER BY dateID asc") // 3. 조회 쿼리 (SQL 직접 작성)
    fun getAllRoutines(): Flow<List<Routine>> // Flow는 데이터 변경 시 실시간으로 알림을 받을 수 있게 함

    @Query("DELETE FROM Routine") // 4. 삭제 쿼리
    suspend fun deleteAll()

    @Query("DELETE FROM Routine where routineID = :id") // 4. 삭제 쿼리
    suspend fun delete(id: Int)

    @Query("SELECT dateID FROM RoutinaryDate WHERE dateID = :id LIMIT 1")
    suspend fun getParentDateIDById(id: String): String?

    @Query("SELECT EXISTS(SELECT 1 FROM Routine WHERE dateID = :id)")
    suspend fun hasDateID(id: String): Boolean

    @Query("SELECT EXISTS(SELECT 1 FROM Routine WHERE routineID = :id)")
    suspend fun hasRoutineID(id: Int): Boolean

    @Query("SELECT MAX(routineID) FROM Routine")
    fun getMaxNumber(): Flow<Int?>

    @Query("UPDATE Routine SET routineTtile = :newTitle, routineContent = :newContent, alarmAllow = :newFlag, alarmTime = :newTime WHERE routineID = :id")
